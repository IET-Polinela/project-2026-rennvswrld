...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # Coba akses detail draft milik Warga B
        url = f'/api/report/{self.draft_milik_b.pk}/'
        response = self.client.get(url)

        # Verifikasi: sistem "berpura-pura" data tidak ada (404), bukan 403
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Warga A seharusnya tidak bisa melihat draf milik Warga B (HTTP 404)"
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        # Payload modifikasi yang dicoba dikirim Warga A
        payload = {
        ...
        # Verifikasi: ditolak dengan 404 (objek dianggap tidak ada di queryset Warga A)
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Warga A seharusnya tidak bisa memodifikasi draf milik Warga B (HTTP 404)"
        )

        # Verifikasi: data asli di database TIDAK berubah sama sekali
        self.draft_milik_b.refresh_from_db()
        self.assertEqual(
            self.draft_milik_b.title,
            'Draf Rahasia Warga B',
            "Konten draf asli milik Warga B tidak boleh berubah"
        )