...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        # (Portal admin pakai Django session, bukan JWT, jadi pakai self.client.login())
        self.client.login(username='warga_test', password='Password123!')

        # LANGKAH 2: Coba akses halaman dashboard admin
        response = self.client.get('/dashboard/')

        # LANGKAH 3: Verifikasi warga biasa ditolak dengan redirect (302)
        self.assertEqual(
            response.status_code,
            302,
            "Warga biasa yang mencoba akses /dashboard/ seharusnya di-redirect (HTTP 302)"
        )