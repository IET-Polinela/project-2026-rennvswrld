...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            'title': 'Lampu Jalan Mati di Gerbang Utama',
            'category': 'Infrastruktur',
            'description': 'Lampu jalan tidak menyala selama 3 hari berturut-turut.',
        ...
        # Verifikasi: field reporter otomatis terisi oleh perform_create()
        laporan = Report.objects.get(title='Lampu Jalan Mati di Gerbang Utama')
        self.assertEqual(
            laporan.reporter,
            self.warga,
            "Field reporter seharusnya otomatis terisi dengan user yang sedang login"
        )
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        payload = {
            'category': 'Infrastruktur',
            'description': 'Laporan ini sengaja tidak diberi judul.',
            'location': 'Lokasi Uji FT-02',
        }

        response = self.client.post(url, payload, format='json')

        # Verifikasi: ditolak dengan 400 Bad Request
        self.assertEqual(
            response.status_code,
        ...
        # Verifikasi: pesan error spesifik untuk field title
        self.assertIn(
            'title',
            response.data,
            "Response error seharusnya menyertakan pesan validasi untuk field 'title'"
        )
    ...
    def test_FT_03_ditolak_jika_deskripsi_kosong(self):
        ...
        # Verifikasi: pesan error spesifik untuk field description
        self.assertIn(
            'description',
            response.data,
            "Response error seharusnya menyertakan pesan validasi untuk field 'description'"
        )
