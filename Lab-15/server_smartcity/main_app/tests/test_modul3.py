...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        # Verifikasi: modifikasi ditolak karena status sudah bukan DRAFT
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Modifikasi laporan berstatus REPORTED seharusnya ditolak (HTTP 403)"
        )

        # Verifikasi: data asli di database tetap aman, tidak berubah
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        payload = {
            'title': 'Coba Ubah Laporan Selesai',
        ...
        # Verifikasi: laporan RESOLVED terkunci mutlak, modifikasi ditolak
        self.assertEqual(
            response.status_code,
            status.HTTP_403_FORBIDDEN,
            "Laporan berstatus RESOLVED seharusnya bersifat read-only (HTTP 403)"
        )
        ...
        self.assertEqual(
            ...
            'AC Rusak di Lab',
            "Judul laporan RESOLVED tidak boleh berubah"
        )
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        """
        # Login session-based sebagai admin
        ...
        # View update_status redirect ke '/' setelah sukses (HTTP 302)
        self.assertIn(
            response.status_code,
            [200, 302],
            "Admin mengubah status REPORTED ke VERIFIED seharusnya berhasil (200/302)"
        )

        # Verifikasi: status di database benar-benar berubah jadi VERIFIED
        self.laporan_reported.refresh_from_db()
        self.assertEqual(
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        # Admin mencoba lompat status secara ilegal: REPORTED -> RESOLVED
        response = self.client.post(url, {'status': 'RESOLVED'})

        # Verifikasi: status TIDAK boleh berubah jadi RESOLVED
        self.laporan_reported.refresh_from_db()
        self.assertNotEqual(
            self.laporan_reported.status,
            'RESOLVED',
        ...
        # Verifikasi tambahan: status tetap di REPORTED (transisi ditolak, bukan diam-diam diubah ke status lain)
        self.assertEqual(
            self.laporan_reported.status,
            'REPORTED',
            "Status seharusnya tetap REPORTED karena transisi ke RESOLVED tidak valid"
        )