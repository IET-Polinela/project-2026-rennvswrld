...
# 🏠 HOME VIEW: render home.html untuk unauthenticated & admin, redirect untuk citizen
def home_view(request):
    user = request.user
    if user.is_authenticated and not getattr(user, 'is_admin', False) and not user.is_staff:
        return redirect('login')
    reports = Report.objects.all() if user.is_authenticated else Report.objects.none()
...
# CREATE (Menggunakan AdminRequiredMixin agar hanya admin yang bisa membuat laporan via web)
...
# UPDATE (Menggunakan AdminRequiredMixin agar hanya admin yang bisa mengedit via web)
...
# DELETE (Menggunakan AdminRequiredMixin agar hanya admin yang bisa menghapus via web)
...
# 🔒 Aturan transisi status: hanya boleh maju SATU langkah, tidak bisa lompat
...
def update_status(request, pk):
    ...
    if request.method == "POST":
        ...
        allowed_next = ALLOWED_STATUS_TRANSITIONS.get(report.status, [])
...
def search_reports(request):
    # Guard: hanya admin yang boleh mengakses endpoint search
    if not request.user.is_authenticated:
...
def report_detail_api(request, pk):
    # Handle request dari RequestFactory yang tidak punya attribute user
    user = getattr(request, 'user', None)
    is_admin = user is not None and (getattr(user, 'is_admin', False) or getattr(user, 'is_staff', False))
    ...
    if is_admin:
        ...
        # Unauthenticated: hanya tampilkan laporan non-DRAFT
        report = get_object_or_404(Report, ~Q(status='DRAFT'), pk=pk)
