...
const SPA_URL = 'http://127.0.0.1:5500/smartcity_citizen_spa_24782092/index.html';;
 ...
 * (misal: http://103.151.63.88:8009/api, http://192.168.1.5/api, dll)
...
test.describe('Modul 1: Otorisasi & Sesi (AUTH-04, AUTH-05, AUTH-06)', () => {
    ...
    test('AUTH-05: Token kadaluarsa → interceptor menangani 401 dan redirect ke #login', async ({ page }) => {
        ...
        await page.unroute('http://103.151.63.88:8009/api/**');
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-02: Live Search pada daftar laporan admin berfungsi', async ({ page }) => {
        ...
        console.log(`[UI-02] Hasil pencarian "${searchKeyword}": ${Array.isArray(responseData) ? responseData.length : (responseData.results?.length || 0)} item`);
        ...
        const resultList = Array.isArray(responseData) ? responseData : (responseData.results || []);
        if (resultList.length > 0) {
            expect(filteredRowCount).toBeGreaterThan(0);
            expect(filteredRowCount).toBe(resultList.length);
        }
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        // LANGKAH 2: Handle dialog alert (jika ada)
        // -------------------------------------------------------------------
        // app.js menggunakan showToast() bukan alert(), sehingga tidak ada
        // dialog browser yang muncul. Handler ini tetap dipasang sebagai
        // fallback jika ada dialog yang tidak terduga.
        //
        page.on('dialog', async (dialog) => {
            console.log(`[UI-05] Dialog (unexpected): "${dialog.message()}"`);
            await dialog.accept();
        });
        ...
        await page.locator('#inputCategory').selectOption('INFRASTRUCTURE');
        ...
        // -------------------------------------------------------------------
        // app.js menggunakan showToast() yang merender elemen .t-toast.success
        // di dalam #toast-wrap, bukan alert() browser.
        // Kita tunggu elemen toast muncul dan berisi teks 'berhasil'.
        //
        const toastWrap = page.locator('#toast-wrap');
        await expect(toastWrap.locator('.t-toast.success')).toBeVisible({ timeout: 5000 });
        const toastText = await toastWrap.locator('.t-toast__body').textContent();
        expect(toastText).toContain('berhasil');
